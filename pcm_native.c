...
int snd_pcm_info(struct snd_pcm_substream *substream, struct snd_pcm_info *info)
{
	...
	strlcpy(info->subname, substream->name, sizeof(info->subname));
        pr_err("-----------------------------------------\n");
        pr_err("-----------------------------------------\n");
        pr_err("substream->name:%s \n",substream->name);
        pr_err("info->card:%d,info->device:%d,info->subdevice:%d \n",info->card,info->device,info->subdevice);
        pr_err("info->id:%s,info->name:%s,info->subdevices_count:%d \n",info->id,info->name, info->subdevices_count);
    pr_err("-----------------------------------------\n");
    pr_err("-----------------------------------------\n");
    pr_err("-----------------------------------------\n");
	return 0;
				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	if (!params->msbits) {
		...
		pr_err("SNDRV_PCM_HW_PARAM_SAMPLE_BITS \n");
		if (snd_interval_single(i)){
			pr_err("snd_interval_single(i) :%d \n",snd_interval_single(i) );
			pr_err("params->msbits : %d \n",snd_interval_value(i) );
	...
	if (!params->rate_den) {
		...
		if (snd_interval_single(i)) {
			pr_err("SNDRV_PCM_HW_PARAM_RATE \n");
			pr_err("params->rate_num:%d \n ",snd_interval_value(i) );
	...
	if (!params->fifo_size) {
		...
		if (snd_mask_single(m) && snd_interval_single(i)) {
			pr_err("SNDRV_PCM_IOCTL1_FIFO_SIZE \n");
				 ...
				 unsigned int cmd, void __user *arg)
{
	...
	pr_err("SNDDRV, snd_pcm_common_ioctl :%x \n", cmd);
	switch (cmd) {
		      ...
		      struct vm_area_struct *area)
{
	...
	if (substream->ops->mmap)
		err = substream->ops->mmap(substream, area);
               ...
               pr_err("snd_pcm_lib_default_mmap \n");
...
static int snd_pcm_mmap(struct file *file, struct vm_area_struct *area)
{
	...
	case SNDRV_PCM_MMAP_OFFSET_STATUS:{
		pr_err("SNDRV_PCM_MMAP_OFFSET_STATUS\n");
		if (!pcm_status_mmap_allowed(pcm_file)) {
		        pr_err("pcm_status_mmap_allowed failed\n");
			return -ENXIO;
		}
		return snd_pcm_mmap_status(substream, file, area);}
	case SNDRV_PCM_MMAP_OFFSET_CONTROL:{
						   pr_err("SNDRV_PCM_MMAP_OFFSET_CONTROL\n");
		if (!pcm_control_mmap_allowed(pcm_file)){
			pr_err("pcm_control_mmap_allowed failed \n");
			return -ENXIO;
		}
		return snd_pcm_mmap_control(substream, file, area);}
	default:{
			pr_err("snd_pcm_mmap_data default \n");
