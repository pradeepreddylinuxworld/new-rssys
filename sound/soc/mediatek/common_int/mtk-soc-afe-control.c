...
{
	unsigned int Audio_I2S2_In = 0;
    unsigned int Audio_I2S1_Out = 0;
	unsigned int sampleRateType;
    
    
    //printk("DigtalI2S->mI2S_SAMPLERATE = %d\n", DigtalI2S->mI2S_SAMPLERATE);
	sampleRateType = SampleRateTransform(DigtalI2S->mI2S_SAMPLERATE,
					     Soc_Aud_Digital_Block_I2S_IN_ADC);
    
    #if 0
    printk("DigtalI2S->mLR_SWAP = %d, DigtalI2S->mBuffer_Update_word = %d, DigtalI2S->mINV_LRCK = %d \
    DigtalI2S->mFpga_bit_test = %d, DigtalI2S->mFpga_bit = %d, DigtalI2S->mloopback = %d, sampleRateType = %d,\
    DigtalI2S->mI2S_FMT = %d, DigtalI2S->mI2S_WLEN = %d\n", DigtalI2S->mLR_SWAP, DigtalI2S->mBuffer_Update_word, DigtalI2S->mINV_LRCK,
    DigtalI2S->mFpga_bit_test, DigtalI2S->mFpga_bit, DigtalI2S->mloopback, sampleRateType, DigtalI2S->mI2S_FMT, DigtalI2S->mI2S_WLEN);
    #endif
    /* Set I2S1 OUT*/
    Audio_I2S1_Out |= (DigtalI2S->mLR_SWAP << 31);
	Audio_I2S1_Out |= (DigtalI2S->mBuffer_Update_word << 24);
	Audio_I2S1_Out |= (DigtalI2S->mINV_LRCK << 23);
	Audio_I2S1_Out |= (DigtalI2S->mFpga_bit_test << 22);
	Audio_I2S1_Out |= (DigtalI2S->mFpga_bit << 21);
	Audio_I2S1_Out |= (DigtalI2S->mloopback << 20);
	Audio_I2S1_Out |= (sampleRateType << 8);
	Audio_I2S1_Out |= (DigtalI2S->mI2S_FMT << 3);
	Audio_I2S1_Out |= (DigtalI2S->mI2S_WLEN << 1);
    pr_debug("%s Audio_I2S1_Out = 0x%x\n", __func__, Audio_I2S1_Out);
	Afe_Set_Reg(AFE_I2S_CON1, Audio_I2S1_Out, MASK_ALL);
                         

	/* Set I2S_ADC_IN */
	Audio_I2S2_In |= (DigtalI2S->mLR_SWAP << 31);
	Audio_I2S2_In |= (DigtalI2S->mBuffer_Update_word << 24);
	Audio_I2S2_In |= (DigtalI2S->mINV_LRCK << 23);
	Audio_I2S2_In |= (DigtalI2S->mFpga_bit_test << 22);
	Audio_I2S2_In |= (DigtalI2S->mFpga_bit << 21);
	Audio_I2S2_In |= (DigtalI2S->mloopback << 20);
	Audio_I2S2_In |= (sampleRateType << 8);
	Audio_I2S2_In |= (DigtalI2S->mI2S_FMT << 3);
	Audio_I2S2_In |= (DigtalI2S->mI2S_WLEN << 1);
	pr_debug("%s Audio_I2S2_In = 0x%x\n", __func__, Audio_I2S2_In);
	Afe_Set_Reg(AFE_I2S_CON2, Audio_I2S2_In, MASK_ALL);
    
    SetCLkMclk(Soc_Aud_I2S1, DigtalI2S->mI2S_SAMPLERATE);
    SetCLkMclk(Soc_Aud_I2S2, DigtalI2S->mI2S_SAMPLERATE);
...
{
    pr_debug("SetExtI2SAdcInEnable bEanble = %d\n", bEnable);
...
bool set_adc2_enable(bool enable)
{
	...
	if (enable) {
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_ADDA_UL2)) {
				pr_info("%s(), GetMemoryPathEnable enable = %d\n",
                    __func__, GetMemoryPathEnable(Soc_Aud_Digital_Block_ADDA_UL2));
			...
			Afe_Set_Reg(AFE_ADDA6_UL_SRC_CON0,0x4 << 27, 0x4 << 27);
		}
#ifdef CONFIG_FPGA_EARLY_PORTING
		pr_debug("%s(), enable fpga clock divide by 4", __func__);
		Afe_Set_Reg(FPGA_CFG0, 0x1 << 1, 0x1 << 1);
