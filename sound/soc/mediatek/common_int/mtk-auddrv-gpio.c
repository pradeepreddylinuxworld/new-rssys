...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_AUD_AP_DMIC_ON] = {"aud_ap_dmic_on", false,
		NULL
	},
	[GPIO_AUD_AP_DMIC_OFF] = {"aud_ap_dmic_off", false,
		NULL
	},
};
#endif
...
static int set_aud_ap_dmic(bool _enable)
{
#if 1//ndef CONFIG_MTK_TINYSYS_SCP_SUPPORT
	static int aud_ap_dmic_counter;

	if (_enable) {
		aud_ap_dmic_counter++;
		if (aud_ap_dmic_counter == 1)
			return AudDrv_GPIO_Select(GPIO_AUD_AP_DMIC_ON);
	} else {
		if (aud_ap_dmic_counter > 0) {
			aud_ap_dmic_counter--;
		} else {
			aud_ap_dmic_counter = 0;
			pr_info("%s(), counter %d <= 0\n", __func__,
				aud_ap_dmic_counter);
		}

		if (aud_ap_dmic_counter == 0)
			return AudDrv_GPIO_Select(GPIO_AUD_AP_DMIC_OFF);
	}
#endif
	return 0;
}
