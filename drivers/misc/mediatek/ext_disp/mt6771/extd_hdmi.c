...
struct notify_dev hdmi_audio_data;
...
int notify_uevent_audio(struct notify_dev *sdev, int state)
{
	char *envp[3];
	char name_buf[120];
	char state_buf[120];
...
void hdmi_state_callback(enum HDMI_STATE state, int audio_support)
{
        static int hdmi_audio = 0;
	...
	case HDMI_STATE_NO_DEVICE:
	{
		 ...
		 */
#endif
		notify_uevent_audio(&hdmi_audio_data, audio_support);
		hdmi_suspend();
		if(hdmi_audio) {
		extcon_set_state_sync(hdmi_extcon,
				EXTCON_DISP_HDMI, HDMI_STATE_NO_DEVICE);
...
int hdmi_init(void)
{
	...
	hdmi_audio_data.name = "audio";
	hdmi_audio_data.index = 0;
	hdmi_audio_data.state = 0;
	ret = hdmi_uevent_dev_register(&hdmi_audio_data);
