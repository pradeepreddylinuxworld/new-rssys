...
#define EM_GSENSOR	"[gsensor-stk8baxx] "
#ifdef CONFIG_RECORD_KLOG
void printc(const char *fmt, ...);
#define PRINT_GSENSOR(fmt, args...)   printc(EM_GSENSOR fmt,##args)
#else
#define PRINT_GSENSOR(fmt, args...)   printk(EM_GSENSOR fmt,##args)
#endif
...
static int stk8baxx_resume_flag = 0;
...
static int stk_get_pid(struct stk8baxx_data *stk)
{
    ...
    if (error)
        PRINT_GSENSOR("failed to read PID\n");
    else{
		stk->pid = (int)val;
		PRINT_GSENSOR("%s: pid = %d\n",__FUNCTION__,stk->pid);
	}
    return error;
}
...
static void stk_read_accel_rawdata(struct stk8baxx_data *stk)
{
    u8 dataL = 0;
    u8 dataH = 0;
    if(!stk8baxx_resume_flag)
    {
		//STK_ACC_ERR("stk_read_accel_rawdata skip, not resumed...");
    	return;
    }
...
/**
 * @brief: Read stk8baxx_data.recv(from recv_store), then send to userspace.
 *
 * @param[in] ddri: struct device_driver *
 * @param[in/out] buf: char *
 *
...
static int stk8baxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	PRINT_GSENSOR("%s in.\n",__func__);
    printk("%s: driver version:%s\n",__FUNCTION__,STK_ACC_DRIVER_VERSION);
	...
	PRINT_GSENSOR(" client->addr=0x%x,hw.direction=%d\n",client->addr,stk->hw.direction);
    ...
    if (stk_reg_init(stk))
    {
        PRINT_GSENSOR("stk8baxx initialization failed");
...
if(0)
{
    ...
    if (error)
    {
        PRINT_GSENSOR("stk8baxx misc_register failed");
    ...
    if (error)
    {
        PRINT_GSENSOR("stk_create_attr failed");
        goto exit_misc_error;
    }
    ...
    if (error)
    {
        PRINT_GSENSOR("accel_factory_device_register failed");
        goto exit_misc_error;
    }
    ...
    if (error)
    {
        PRINT_GSENSOR("acc_register_control_path fail");
    ...
    if (error)
    {
        PRINT_GSENSOR("acc_register_data_path fail");
        goto exit_register_control_path;
    }
...
static int stk_resume(struct device *dev)
{
	...
	STK_ACC_ERR("begin resume stk8baxx...");
   //  mdelay(35);
    if (NULL == stk)
    {
        STK_ACC_ERR("Null point to find stk8baxx_data");
    ...
    if (STK8BAXX_PWMD_SUSPEND != stk->power_mode)
    {
        ...
        if (stk8baxx_reg_read(stk, STK8BAXX_REG_POWMODE, 0, &val)) {
		    STK_ACC_ERR("end resume stk8baxx...1nd");
			stk8baxx_resume_flag = 1;
            return -EINVAL;
			}
        ...
        if (stk8baxx_reg_write(stk, STK8BAXX_REG_POWMODE, (val | stk->power_mode))) {
			STK_ACC_ERR("end resume stk8baxx...2nd");
			stk8baxx_resume_flag = 1;
            return -EINVAL;
			}
    }
    //mdelay(35);
	STK_ACC_ERR("end resume stk8baxx...");
	stk8baxx_resume_flag = 1;
    return 0;
}
...
{
    {.compatible = "mediatek,gsensor-stk8baxx"},
    {},
};
