...
#define PRINT_MSENSOR(fmt, args...)   printc(MMC5603NJ_DEV_NAME fmt,##args)
#else
#define PRINT_MSENSOR(fmt, args...)   printk(MMC5603NJ_DEV_NAME fmt,##args)
#endif
...
static int mmc5603x_get_trim_data()
{
	uint8_t reg_value[3];
...
*********************************************************************************/
static int mmc5603x_auto_selftest()
{
...
static void mmc5603x_read_mode_switch(float raw_data_ut[])
{
	unsigned char reg[2] = {0};
	...
	if (MMC5603_Read_mode == CONTINUOUS_MODE_AUTO_SR)
	{
		...
		if ((fabs(raw_data_ut[0]) > 10000.0f) || (fabs(raw_data_ut[1]) > 10000.0f))
		{
	...
	else if (MMC5603_Read_mode == SINGLE_MODE)
	{
		...
		if ((fabs(raw_data_ut[0]) < 8000.0f) && (fabs(raw_data_ut[1]) < 8000.0f))
		{
...
static int check_device(void)
{
	...
	{
		PRINT_MSENSOR("[mmc5603nj] read id fail\n");
		//read again
		I2C_RxData(product_id, 1);
	}

	PRINT_MSENSOR("[mmc5603nj] product_id[0] = %d\n", product_id[0]);
	if (product_id[0] != MMC5603NJ_PRODUCT_ID)
	{
		PRINT_MSENSOR("Got memsic mmc5603nj id failed");
		return -1;
	}
	return 0;
}
...
static int mmc5603nj_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("%s: enter probe,driver version=%s\n", __func__, DRIVER_VERSION);
	...
	if (err < 0)
	{
		PRINT_MSENSOR("get dts info fail\n");
		err = -EFAULT;
		goto exit;
	}
	//data->hw.direction = 15;
	...
	PRINT_MSENSOR("client->addr =0x%x,direction =%d\n", client->addr,data->hw.direction);

	err = check_device();
	if (err < 0)
	{
		PRINT_MSENSOR("mmc5603x probe: check device connect error\n");
		goto exit_kfree;
	}
	...
	PRINT_MSENSOR("mmc5603NJ IIC probe successful !\n");

	mmc5603nj_init_flag = 0;
	return 0;
