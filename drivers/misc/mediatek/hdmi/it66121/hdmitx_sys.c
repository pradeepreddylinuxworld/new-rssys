...
#include "hdmitx.h"
#include "../../sensors-1.0/hall/lid-switch.h"
/* #include <linux/switch.h> */

DECLARE_WAIT_QUEUE_HEAD(waiter3);
DECLARE_WAIT_QUEUE_HEAD(waiter4);
 int audio_supported;
 bool hdmi_connected=0;
 bool hdmi_disconnected=0;
 _CODE HDMITXDEV
	InstanceData = {
...
/* HDMITX_DisableVideoOutput(); */
			...
			if (hdmi_util.state_callback) {
				hdmi_connected = 1;
				wake_up(&waiter3);
				hdmi_util.state_callback(1, audio_supported);
				hdmi_invoke_cable_callbacks(HDMI_STATE_ACTIVE,audio_supported);
			}
			...
			if (hdmi_util.state_callback) {
				...
				hdmi_connected = 0;
				wake_up(&waiter4);
				hdmi_util.state_callback(0, 0);
				hdmi_invoke_cable_callbacks(
					HDMI_STATE_NO_DEVICE, 0);
			}
			it66121_bChangeAudio = FALSE;
		}
	} else {
