...
#if 0//defined(GPIO_HDMI_9024_RESET)
...
void hdmi_invoke_cable_callbacks(enum HDMI_STATE state, int audio_support)
{
	int i = 0, j = 0;
	...
	if (hdmi_callback_table[j]) {
		...
		hdmi_callback_table[j](state,audio_support);
	}
}
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(hdmi_vcn33))
       ...
       regulator_set_voltage(hdmi_vcn33, 3300000, 3300000);
