...
#endif
extern int get_dc_det_pin_status(void);
...
#endif
    int boot_mode = 0,dc_status = -1, vbus_status = -1;
    
	boot_mode = get_boot_mode();
    dc_status = get_dc_det_pin_status();
    vbus_status = get_vbus_det_pin_status();

	mutex_lock(&chrdet_lock);
    pr_info("do_charger_detect: boot_mode = %d,usb_swith_to_host = %d, dc_status = %d, vbus_status = %d\n", boot_mode, usb_swith_to_host(),dc_status,vbus_status);
    
	if (pmic_get_register_value(PMIC_RGS_CHRDET)) 
    {
        if (boot_mode == NORMAL_BOOT)
        {
            if (usb_swith_to_host() == 0)//usb
            {
                if (1 == dc_status) 
                {
                    pr_info("charger type: device DC IN\n");
                    g_chr_type = STANDARD_CHARGER;
                    chrdet_inform_psy_changed(g_chr_type, 1);     
                }
                else if(1 == vbus_status)
                {
                    pr_info("charger type: device usb in\n");
                    g_chr_type = STANDARD_HOST;//usb
                    chrdet_inform_psy_changed(g_chr_type, 1);
                }
            }
            else if(usb_swith_to_host() == 1)
            {
                if (1 == dc_status)
                {
                    pr_info("charger type: host DC in\n");
                    g_chr_type = STANDARD_CHARGER;
                    chrdet_inform_psy_changed(g_chr_type, 1);
                }
                else
                {
                    pr_info("charger type: host usb in or out\n");
                    g_chr_type = CHARGER_UNKNOWN;
                    chrdet_inform_psy_changed(g_chr_type, 0);
                }
            }
            else
                pr_err("switch to host or usb failed\n");
        }
        else //poweroff charger, FACTORY_BOOT = 4, KERNEL_POWER_OFF_CHARGING_BOOT = 8
        {
            if(1 == get_dc_det_pin_status())
            {
                pr_info("charger type: poweroff chareger DC in\n");
                g_chr_type = STANDARD_CHARGER;
                chrdet_inform_psy_changed(g_chr_type, 1);
            }
            else
            {
                pr_info("charger type: poweroff chareger usb in\n");
