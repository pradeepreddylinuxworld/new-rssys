...
extern void set_lcm_backlight_on(void);
		       ...
		       unsigned long arg)
{
	...
	case MTKFB_SET_AOD_POWER_MODE:
	{
		...
		if (aod_pm == MTKFB_AOD_DOZE_SUSPEND) {
			pr_err("pradeep, MTKFB_AOD_DOZE_SUSPEND \n");
			/*
			...
			if (primary_display_is_sleepd() &&
				...
				pr_err("pradeep, primary_display_set_power_mode,MTKFB_AOD_DOZE_SUSPEND \n");
			...
			pr_err("pradeep, primary_display_set_power_mode, MTKFB_AOD_DOZE \n");
			primary_display_set_power_mode(DOZE);
	...
	case MTKFB_POWEROFF:
	{
		...
		pr_err("pradeep, [FB Driver] enter MTKFB_POWEROFF\n");
		pr_err("pradeep, [FB Driver] leave MTKFB_POWEROFF\n");
	...
	case MTKFB_POWERON:
	{
		...
		pr_err("pradeep, [FB Driver] enter MTKFB_POWERON\n");
		pr_err("pradeep, [FB Driver] leave MTKFB_POWERON\n");
	...
	{
		int power_state;
                pr_err("pradeep, MTKFB_GET_POWERSTATE \n");
		if (primary_display_is_sleepd()){
			pr_err("pradeep, MTKFB_GET_POWERSTATE, power_state = 0 \n");
			power_state = 0;
		}
		else {
			pr_err("pradeep, MTKFB_GET_POWERSTATE, power_state = 1 \n");
			power_state = 1;
		}
	...
	case MTKFB_META_RESTORE_SCREEN:
	{
		...
		if (copy_from_user(&var, argp, sizeof(var)))
			return -EFAULT;
                pr_err("pradeep, MTKFB_META_RESTORE_SCREEN \n");
			      ...
			      unsigned long arg)
{
	...
	case COMPAT_MTKFB_GET_DISPLAY_IF_INFORMATION:
	{
		...
		pr_err("pradeep, COMPAT_MTKFB_GET_DISPLAY_IF_INFORMATION, displayid:%d \n", displayid);
		if (displayid == 0) {
			dispif_info[displayid].displayWidth =
						primary_display_get_width();
			dispif_info[displayid].displayHeight =
			...
			DDPPR_ERR("pradeep,information for displayid: %d is not available now\n",
			displayid);
		}

		if (copy_to_user((void __user *)arg, &(dispif_info[displayid]),
				 sizeof(struct compat_mtk_dispif_info))) {
			pr_err("pradeep, [FB]: copy_to_user failed! line:%d\n",
			       __LINE__);
			return -EFAULT;
		}
		break;
	...
	case COMPAT_MTKFB_GET_POWERSTATE:
	{
		...
		data32 = compat_ptr(arg);
		if (primary_display_is_sleepd()){
		       pr_err("pradeep, COMPAT_MTKFB_GET_POWERSTATE power_state = 0 \n");
			power_state = 0;
		}
		else{
			pr_err("pradeep, COMPAT_MTKFB_GET_POWERSTATE power_state = 1 \n");
			power_state = 1;
		}
