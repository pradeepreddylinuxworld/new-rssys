...
//for cw2015 fuel fauge
#if defined(CONFIG_I2C_CW2015_FUEL_GAUGE)
	...
	union power_supply_propval *val)
{
    ...
    //0% powr off
#if defined(CONFIG_I2C_CW2015_FUEL_GAUGE)    
    data->BAT_CAPACITY = g_cw2015_capacity;
#endif
...
static int bat_temperature_val_force = 0;
int force_get_tbat_internal(bool update)
{
	...
	bat_temperature_val = bat_temperature_val_force?bat_temperature_val_force:bat_temperature_val;
...
static ssize_t show_bat_temperature_val_force(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	bm_trace(
		"[FG] show bat_temperature_val_force : %d\n",
		bat_temperature_val_force);
	return sprintf(buf, "%d\n", bat_temperature_val_force);
}

static ssize_t store_bat_temperature_val_force(
	struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	unsigned long val = 0;
	...
	if (buf != NULL && size != 0) {
		...
		if (val < 0) {
			...
			bat_temperature_val_force = val;

		bm_err(
			"[%s] bat_temperature_val_force = %d\n",
			__func__,
			(int)val);
	}
...
static DEVICE_ATTR(
	bat_temperature_val_force, 0664,
	show_bat_temperature_val_force, store_bat_temperature_val_force);
...
/*****************************/
	...
	ret_device_file = device_create_file(&(dev->dev),
		&dev_attr_bat_temperature_val_force);
	ret_device_file = device_create_file(&(dev->dev),
...
void battery_shutdown(struct platform_device *dev)
{
	...
	int shut_car_diff = 0;
	int verify_car = 0;
	printk("hechuan:>>>>>>>>>>>battery_shutdown");
        dump_stack();
