...
static struct work_struct dc_det_wq;
static unsigned int GPIO_DC_DET;
static unsigned int GPIO_VBUS_DET;
static int dc_eint_num;
...
int get_dc_det_pin_status(void)
{
    return gpio_get_value(GPIO_DC_DET);
}

int get_vbus_det_pin_status(void)
{
    return gpio_get_value(GPIO_VBUS_DET);
}

extern void chrdet_int_handler(void); 
static void dc_det_pin_delayed_work(struct work_struct *data)
{
    int dc_gpio_val;
	
    dc_gpio_val = gpio_get_value(GPIO_DC_DET);
    if (1 == dc_gpio_val)
	{
		irq_set_irq_type(dc_eint_num, IRQF_TRIGGER_LOW);
		enable_irq(dc_eint_num);
	}
    else
    {		
		irq_set_irq_type(dc_eint_num, IRQF_TRIGGER_HIGH);
		enable_irq(dc_eint_num);
    }
...
static irqreturn_t dc_int_handler(int irq, void *dev_id)
{   
    //printk("dc_int_handler>>>>>>\n");
	//queue_delayed_work(dc_det_wq, &dc_det_pin_work, msecs_to_jiffies(1));
    disable_irq_nosync(dc_eint_num);
	schedule_work(&dc_det_wq);
	return IRQ_HANDLED;
}
   
static int charger_gpio_detect_init(struct platform_device *pdev)
{
    int ret;
    ...
    chrdet_int_handler();
    INIT_WORK(&dc_det_wq, dc_det_pin_delayed_work);

    GPIO_VBUS_DET = of_get_named_gpio(dev->of_node, "gpio_vbus_det", 0);
    if (gpio_is_valid(GPIO_VBUS_DET))
    {
	    pr_debug("get GPIO_VBUS_DET = %d success\n", GPIO_VBUS_DET);
        ret = gpio_request(GPIO_VBUS_DET, "GPIO_VBUS_DET");
	    if(ret < 0)
	    {
            chr_err("request GPIO_VBUS_DET failed\n");
            gpio_free(GPIO_VBUS_DET);
        }
	    gpio_direction_input(GPIO_VBUS_DET);
    }
    else
    {
	    GPIO_VBUS_DET = -1;
	    chr_err("of get GPIO_VBUS_DET failed\n");
    }
    
	GPIO_DC_DET = of_get_named_gpio(dev->of_node, "gpio_dc_det", 0);
	if ((gpio_is_valid(GPIO_DC_DET)))
	{
		pr_debug("get GPIO_DC_DET = %d success\n",GPIO_DC_DET);
		ret = gpio_request(GPIO_DC_DET, "GPIO_DC_DET");
		if(ret < 0)
		{
			chr_err("request GPIO_DC_DET failed\n");
			gpio_free(GPIO_DC_DET);
		}	
		gpio_direction_input(GPIO_DC_DET);
	}
	else
	{
		gpio_free(GPIO_DC_DET);
		chr_err( "of get GPIO_DC_DET failed\n");
    }
    
    gpio_set_debounce(GPIO_DC_DET, 64000);
	dc_eint_num = gpio_to_irq(GPIO_DC_DET);
	ret = request_irq(dc_eint_num, dc_int_handler, IRQF_TRIGGER_HIGH, "DC_DET", NULL);
	if (ret) 
	{
		chr_err("request DC-EINT <%d> fail, ret<%d>\n",dc_eint_num, ret);
		return ret;
	}
