...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (pdata->force_charging_current/*==0*/ > 0) {
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)/*==0*/) {
	...
	if (pdata->thermal_charging_current_limit/*==0*/ != -1) {
	...
	if (pdata->thermal_input_current_limit/*==-1*/ != -1) {
	...
	if (mtk_pe40_get_is_connect(info)) {
		if (info->pe4.pe4_input_current_limit != -1 && /*==-1*/
	...
	if (pdata->input_current_limit_by_aicl != -1 && /*==-1*/
	...
	chr_err("force:%d thermal:%d,%d pe4:%d,%d,%d setting:%d %d type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d atm:%d\n",
		_uA_to_mA(pdata->force_charging_current), /*0*/
		_uA_to_mA(pdata->thermal_input_current_limit),/*-1*/
		_uA_to_mA(pdata->thermal_charging_current_limit),/*0*/
		_uA_to_mA(info->pe4.pe4_input_current_limit),/*-1*/
		_uA_to_mA(info->pe4.pe4_input_current_limit_setting),/*-1*/
		_uA_to_mA(info->pe4.input_current_limit),/*0*/
		_uA_to_mA(pdata->input_current_limit),/*3200*/
		_uA_to_mA(pdata->charging_current_limit),/*0*/
		info->chr_type,/*4*/ info->usb_unlimited,/*0*/
		IS_ENABLED(CONFIG_USBIF_COMPLIANCE)/*0*/, info->usb_state/*0*/,
		pdata->input_current_limit_by_aicl/*-1*/, info->atm_enabled)/*0*/;

	if (info->battery_temp >= 60){
		pdata->charging_current_limit = 0;
		pdata->input_current_limit = 128000;
	} else if (info->battery_temp > 45) {
		pdata->charging_current_limit = 700000;
