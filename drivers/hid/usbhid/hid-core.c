...
static bool is_usbmouse_in = false;
extern int otg_iddig_power_suspend;
extern void set_fb_notifier_powerdown(void);
bool get_usbmouse_in_status(void)
{
    return is_usbmouse_in;
}
#endif
...
static void usbhid_disconnect(struct usb_interface *intf)
{
	...
	if (WARN_ON(!hid))
		return;
#ifdef CONFIG_USBMOUSE_IN_NO_POWEROFF_SUSPEND
    //printk("%s : hid->type = %d, otg_iddig_power_suspend = %d\n", __func__, hid->type, otg_iddig_power_suspend);
    if(hid->type == HID_TYPE_USBMOUSE)
    {
        is_usbmouse_in = false;
        if (otg_iddig_power_suspend == 0)
            set_fb_notifier_powerdown();
    }
#endif
