				...
				bool first_packet)
{
	...
	if (hid->claimed & HID_CLAIMED_INPUT) {
        ...
        case 0x90001 :
            //printk("case : 0x90001  usage->type = %d, usage->code = 0x%x, value = %d, touch_x = 0x%x touch_y = 0x%x\n",usage->type, usage->code, value, touch_x,touch_y);
            if (usage->type == 1 && touch_x > 0x2c3 && value == 1)
            {
               // printk(">>>>>>>key 0x111 value = %d\n",value);
			   // input_event(input, usage->type, 0x110, 0);
				input_event(input, usage->type, 0x111, 1);
            }
            else if (usage->type == 1 && value == 1)
            {
                //printk(">>>>>>>>>>>key 0x110 value = %d\n",value);
				//input_event(input, usage->type, 0x111, 0);
                input_event(input, usage->type, 0x110, 1);
            }
			else if (usage->type == 1 && value == 0)
            {
				input_event(input, usage->type, 0x111, 0);
                input_event(input, usage->type, 0x110, 0);
            }
            break;
        case 0x90002 :
            //printk("case : 0x90002 do nothing break, usage->type = %d, usage->code = 0x%x, value = %d\n",usage->type, usage->code, value);
         //   if (usage->type == 1)
		//		input_event(input, usage->type, usage->code, value);
            break;
        case 0x90003 :
            //printk("case : 0x90003 do nothing break, usage->type = %d, usage->code = 0x%x, value = %d\n",usage->type, usage->code, value);
            //if (usage->type == 1)
			//	input_event(input, usage->type, usage->code, value);
            break;
