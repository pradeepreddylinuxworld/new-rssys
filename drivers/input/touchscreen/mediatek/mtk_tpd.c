			...
			unsigned int cmd, unsigned long arg)
{
	...
	case TPD_GET_ATA_PARA:
			...
			if((g_tpd_drv != NULL) && (g_tpd_drv->factory_test))
			{
				printk("g_tpd_drv->factory_test() \n");
				result = g_tpd_drv->factory_test();	
			}
			if(0 == result)
                result = 1;
            else
...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	if (strcmp(tpd_drv->tpd_device_name, "generic") == 0) {
		...
		tpd_driver_list[0].factory_test = tpd_drv->factory_test;
		return 0;
	}
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name == NULL) {
			...
			//add for factory_test by qinghui
			tpd_driver_list[i].factory_test = tpd_drv->factory_test;
			//add end
			break;
		}
...
#if defined(CONFIG_USE_PEN_PRESSURE)
