			       ...
			       const struct usb_device_id *id)
{
	...
	dev_err(ddev, "usb_serial_probe 2\n");
	...
	mutex_unlock(&table_lock);
	dev_err(ddev, "usb_serial_probe 3\n");
	...
	dev_err(ddev, "usb_serial_probe 4\n");
	...
	if (type->probe) {
		...
		retval = type->probe(serial, id);
	        dev_err(ddev, "usb_serial_probe 5\n");

		if (retval) {
			dev_err(ddev, " usb_serial_probe sub driver rejected device\n");
			goto err_put_serial;
		}
	}
	dev_err(ddev, "usb_serial_probe 5\n");
	...
	dev_err(ddev, "usb_serial_probe 6\n");
	...
	if (num_ports > MAX_NUM_PORTS) {
	           dev_err(ddev, "usb_serial_probe too many ports\n");
	...
	dev_err(ddev, "%s converter detected\n", type->description);
...
static int __init usb_serial_init(void)
{
	...
	pr_err("pradeep, usb_register passed for ttyUSB\n");
